...
use gc_arena::{Gc, Collect, MutationContext};
use std::fmt::Debug;
...
impl<'gc> Namespace<'gc> {
    ...
    pub fn package(package_name: impl Into<AvmString<'gc>>, mc: MutationContext<'gc, '_>,) -> Self {
        Self(Gc::allocate(mc, NamespaceData::Namespace(package_name.into())))
    }

    pub fn internal(package_name: impl Into<AvmString<'gc>>, mc: MutationContext<'gc, '_>,) -> Self {
        Self(Gc::allocate(mc, NamespaceData::PackageInternal(package_name.into())))
    }
