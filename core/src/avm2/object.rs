...
pub trait TObject<'gc>: 'gc + Collect + Debug + Into<Object<'gc>> + Clone + Copy {
    ...
    fn get_property(
        ...
        match self.instance_of().and_then(|class| class.get_instance_trait(multiname)) {
            ...
            Some(Property::Method{disp_id, ..}) => {
                ...
                // Try lazily binding from class traits...
                if let Some(class) = self.as_class_object() {
                    if let Some(bound_method) = class.bound_class_method(activation, disp_id) {
                        self.install_bound_method(activation.context.gc_context, disp_id, bound_method);
                        return Ok(bound_method.into());
                    }
                }
                // Then try as normal instance
                if let Some(class) = self.instance_of() {
                    if let Some(bound_method) = class.bound_instance_method(activation, receiver, disp_id)
                    {
                        self.install_bound_method(activation.context.gc_context, disp_id, bound_method);
                        return Ok(bound_method.into());
                    }
    ...
    fn set_property(
        ...
        match self.instance_of().and_then(|class| class.get_instance_trait(multiname)) {
            ...
            Some(Property::Virtual{set: Some(set), ..}) => {
                self.call_method(set, &[], activation).map(|_| ())
            }
    ...
    fn call_property(
        ...
        match self.instance_of().and_then(|class| class.get_instance_trait(multiname)) {
            Some(Property::Method{disp_id, ..}) => {
                if let Some(class) = self.as_class_object() {
                    if let Some(method) = class.get_class_method(disp_id) {
                ...
                if let Some(class) = self.instance_of() {
                    if let Some((superclass, method)) = class.get_full_instance_method(disp_id)
                    {
                        if !method.needs_arguments_object() {
            ...
            None => {
                self.call_property_undef(multiname, arguments, activation)
            }
            _ => {
                let function = self.get_property_local(self.into(), multiname, activation)?
    ...
    fn call_method(
        ...
        if self.base().get_bound_method(id).is_none() {
            if let Some(class) = self.as_class_object() {
                if let Some(bound_method) =
                    class.bound_class_method(activation, id)
                {
            ...
            if let Some(class) = self.instance_of() {
                if let Some(bound_method) =
                    class.bound_instance_method(activation, self.into(), id)
                {
                    self.install_bound_method(activation.context.gc_context, id, bound_method);
                }
            }
        ...
        if let Some(method_object) = self.base().get_bound_method(id) {
    ...
    fn install_const_late(
        ...
        let new_slot_id = self.instance_of().unwrap().install_const_trait_late(mc, name, value.clone());
    ...
    fn install_instance_slots(
        ...
        let slots = from_class_object.default_instance_slots();
