...
pub fn create_class<'gc>(activation: &mut Activation<'_, 'gc>) -> GcCell<'gc, Class<'gc>> {
    ...
    write.define_builtin_instance_methods(mc, activation.avm2().as3_namespace(), PUBLIC_INSTANCE_AND_PROTO_METHODS);

    const PUBLIC_CLASS_METHODS: &[(&str, NativeMethodImpl)] = &[("UTC", utc), ("parse", parse)];

    write.define_builtin_class_methods(mc, activation.avm2().public_namespace(), PUBLIC_CLASS_METHODS);
