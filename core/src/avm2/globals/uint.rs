...
pub fn create_class<'gc>(activation: &mut Activation<'_, 'gc>) -> GcCell<'gc, Class<'gc>> {
    ...
    write.set_native_instance_init(Method::from_builtin_and_params(
        ...
        vec![ParamConfig::of_type("num", Multiname::new(activation.avm2().public_namespace(), "Object"))],
        false,
        mc,
    ));

    const CLASS_CONSTANTS: &[(&str, u32)] = &[("MAX_VALUE", u32::MAX), ("MIN_VALUE", u32::MIN)];
    write.define_constant_uint_class_traits(activation.avm2().public_namespace(), CLASS_CONSTANTS, activation);
